...
The question at the heart of the issue is whether external efficacy is responsive to changes in the political context. If not, then what does the 'external' refer to? Never mind, for a moment, that the meaning of 'efficacy' is all but lost.
...
Efficacy is inherently tied to the end for which some thing or act is purposed. Converse here makes clear that being effective means successfully influencing or being influential in some way in government and politics. Inherent to the notion of political efficacy is the underlying purpose of participating in the political process of whatever system. Implied here by Converse, and by authors everywhere else, is that the purpose of individual or popular political participation is ***to influence*** the governing actors and institutions. Therefore, it is conveyed by implication that *the maximum effect achievable through political activity* is but merely to influence governing actors of the political regime.

The scholars of yesteryear faltered in only this: they presumed the political system and regime as democratic in the same way that every man assumes himself good. In so doing, the maximum effect of the people's popular capacity was not *to govern*, but *to influence governors*; not to rule but to hope that their rulers would pity their grievances. And consequently, scholars of today may still be heard professing elections as a form of accountability, all the while the people flail about searching for the means to govern themselves.
...
The hypothesis posited by Chamberlain still stands: external efficacy should shift upon changes to the political environment.
...
The changes in particular would pertain to conditions most relevant to external efficacy, namely to do with the capacity to influence the behaviors of government officials and outcomes of politics.
...
So long as we accept that a state's cost of voting is, at least in part, a reflection of that state's political environment.
...
$$
The force of gravity is given in meters per square second $(m/s^2)$. The force of gravity on Earth is $9.81 m/s^2$, whereas on the Moon, it is approximately $1.62 m/s^2$. Source [Your Weight on the Moon](https://cosmicopia.gsfc.nasa.gov/qa_earth.html#weight).
:::
...
This is why, as the authors say, the process is, by necessity, theory-laden; then the conception of the construct is theoretical in the first place.
...
Presumably, the aim is to have the respondent evaluate the influence of one's "*say*" on either the actions of governing actors or the resulting outcomes of politics and government (i.e., "what the government does"). We may readily suppose that one's *say* refers to public expression taken as a form of public's attempts at influencing the actions of governing officials[^3].

However, the statement doesn't permit evaluation of the *efficacy* of public input---of expressing one's *say* on the matters at hand. To put in other words, the respondent is not evaluating the means to the end of influencing the acts of government officials and outcomes of politics. Instead, the individual is prompted to consider whether current conditions and government output favor "people like me". 
...
The only way I'm able to get the same number of observations as Chamberlain's Table 1 for model 4 is if I use lags of the prior observation, not t-4 and t-2

```{r}

# why does number of obs in model 4 differ from Chamberlain?
# Chamberlain model 4, n = 21
# replication model 4, n = 19

# lagged by 1 observation, not time
# this is the only way that the number of obs match with chamberlain, except the
# coefficients are completely different. I'm not sure what he did or how he did
# it.
replica |>  
  lm(exteff ~ exteff.L1 + trustgov.L1, data = _) |> 
  summary()
...
jtools::export_summs(
  ...
  coefs = c(
    "External Efficacy (L-1)" = "exteff.L1",
    "Trust in Government (L-1)" = "trustgov.L1",
    "Constant" = "(Intercept)"
  )
)
...
names(models) <- janitor::make_clean_names(names(models), sep_in = " ")

models.tidy <- purrr::map(models, broom::tidy)

list2env(models.tidy, envir = .GlobalEnv)

models.tidy[1] |> str()

og.model.1 <- model.1 |>
  dplyr::mutate(
    estimate = c(13.039, 0.730),
    std.error = c(8.059, 0.149),
    statistic = estimate / std.error,
    p.value = pt(-abs(statistic), df = 21)
  )

og.model.2 <- model.2 |>
  dplyr::mutate(
    estimate = c(24.702, 0.568, -0.048),
    std.error = c(17.436, 0.162, 0.162),
    statistic = estimate / std.error,
    p.value = pt(-abs(statistic), df = 18)
  )

og.model.3 <- model.3 |>
  dplyr::mutate(
    estimate = c(-1.253, 0.719, 0.271),
    std.error = c(10.598, 0.140, 0.140),
    statistic = estimate / std.error,
    p.value = pt(-abs(statistic), df = 20)
  )

og.model.4 <- model.4 |>
  dplyr::mutate(
    estimate = c(17.824, 0.613, 0.033),
    std.error = c(9.343, 0.221, 0.252),
    statistic = estimate / std.error,
    p.value = pt(-abs(statistic), df = 18)
  )
...
og.model.5 <- model.5 |>
  dplyr::mutate(
    estimate = c(18.875, 0.587, -0.132, 0.223),
    std.error = c(17.554, 0.159, 0.170, 0.163),
    statistic = estimate / std.error,
    p.value = pt(-abs(statistic), df = 17)
  )

og.model.6 <- model.6 |>
  dplyr::mutate(
    estimate = c(35.538, 0.383, -0.208, 0.121, 0.169),
    std.error = c(25.520, 0.300, 0.324, 0.206, 0.320),
    statistic = estimate / std.error,
    p.value = pt(-abs(statistic), df = 14)
  )
...
Political efficacy has been conceived as an individual's attitude in regard to themselves situated along two component dimensions, but the conception falters as soon as we move to consider the so-called 'external' dimension. In one dimension, political efficacy is conceived as one's personal confidence in their ability to merely encounter or confront political topics, issues, or affairs; internal political efficacy is no more than an individual's self-qualification as more or less politically competent. The conception of the other dimension, external political efficacy, immediately forgoes an appraisal of the means of political action available to the individual, and as such is not related to political efficacy at all. Instead, external efficacy is referred to as a belief about government responsiveness to public demands. Even if the definition is focused squarely on a person's beliefs about the efficacy of public demands, the wording of the survey items do not prompt an individual to evaluate the efficacy of public influence attempts.  
...
@craig1990's analysis of the 1987 NES pilot study---where they examined 35 different efficacy and trust items to empirically distinguish between political efficacy and trust in government---included at least four items that are far more theoretically viable as items indicative of political efficacy than any other (arguably, even internal efficacy). At least conceptually. They conducted a confirmatory factor analysis with four items meant to capture external efficacy, but the items they selected do not coherently correspond to a valid interpretation of responses. That is, the responses to the items cannot all be validly interpreted as reflecting beliefs about the efficacy of public intervention on the political process to influence governing actors. A valid argument could be made to always include NOSAY as an item composing the external efficacy scale for continuity sake if for nothing else, at least four other items are more theoretically viable as indicants of an underlying political efficacy construct.

Drawing from...

These items are more conceptually coherent as efficacy evaluations than any other. For LEGAL and MAKELSTN, respondents assess and report the extent to which they believe that there are ***means available*** to them to 1) influence what the government does, 2) influence the attention of government officials. "There are many legal ways..." and "...there really is no way to..." can be rephrased as "There are many legal means..." and "...there are really no means to...", which implies that the respondent must consider and judge whether there are means available to the people ***to influence*** government action and outcomes. These are clearly and coherently items that pertain to 'efficacy' more so than many of the others.

As for the third item, VOTESAY is irrefutably an efficacy evaluation. Responses to this item can only be interpreted as an ***evaluation of the efficacy of voting as a means*** toward the stated end. Now the end stated as, “…for people to have a say about what the government does” leads us to understand that voting is evaluated as a form of expression---a form of expressing grievances, opinions, preferences, demands, and so on---and so respondents evaluate the efficacy of one form of expression towards influencing the actions or outcomes of government.

A fourth item that was included in the traditional CPS Responsiveness scale, ELECRESP, is understood more coherently as pertaining to efficacy beliefs than of perceived government responsiveness. The statement reads, “And how much do you feel that having elections makes the government pay attention to what the people think---a good deal, some, or not much?”  

The ELECRESP item statement **focuses on the efficacy of having elections**, which is much more coherent with the ordinary meaning of ‘efficacy’ in the first place---the capacity to produce or achieve intended ends. That item statement prompts an individual to consider the efficacy of elections at achieving a particular end---making the government pay attention to what the people think. Or even more pointedly, the individual considers the extent to which “having elections” facilitates or functions as a means to direct government attention to popular grievance, preferences, demands, etc. So much like the VOTING item, respondents report efficacy beliefs by way of evaluating their capacity to achieve some end by means of the institution designed for that purpose.

The items VOTESAY and ELECRESP clearly follow from the LEGAL and MAKELSTN items; where the LEGAL and MAKELSTN items prompt individuals to report whether there are any means available to them to achieve certain ends, ELECRESP and VOTESAY prompts individuals to report their appraisal of the most prominent means purposed toward such ends.  

In these four items, respondents are at once indicating the extent they believe there to be means available to the people or citizens *writ large*, and the efficacy of certain means, all towards the end of influencing government officials---influencing the actions they take or what they attend to. Thus, these four items are theoretically viable to stand as indicators of a political efficacy belief that generalizes across survey item responses.

Empirically, however, it is unclear given that the original pilot study by @craig1990 did not consider these items to compose a cohesive construct. The average correlation between the six RBE items (LEGAL; FEWPOWER; VOTESAY; FINALSAY; MAKELSTN; NOSAY) was 0.29, which already seems abysmal. The authors stated that adding in two CPS responsiveness items (GOVRESP; ELECRESP) reduces that correlation down to 0.26. However, bear in mind that FEWPOWER, FINALSAY, and GOVRESP do not prompt the individual to appraise a means to some stated end and thus responses cannot be validly considered to reflect efficacy beliefs.

Although FEWPOWER and FINALSAY may reflect an individual’s overarching perspective on conditions of political power within the given country (i.e., “In this country”) or regime (i.e., “Under our form of government…”), item responses reflect one’s perspective on the contemporary political conditions, and not an appraisal of the people’s capacity subject to such conditions. The GOVRESP item is also not an evaluation regarding the efficacy of public input or means thereof, but rather reflects the extent to which government officials take heed of “what people think”, generally speaking, during the process of decision making. The bottom line being that the individual is not prompted to evaluate or appraise the efficacy of the means of political production available to themselves or the people writ large in either of the three items: FEWPOWER, FINALSAY, or GOVRESP.


## 2025-11-17

Note in the 1987 NES Pilot, some (external) efficacy questions were pre-faced with the following

> "We are interested in people's opinions about how government works in general in the United
States and how much influence ordinary people have over it. I am going to read some of the
things people have said about the type of government we have in this country. After I
read each statement, please tell me whether you strongly agree, somewhat agree, neither
agree nor disagree, somewhat disagree, or strongly disagree."

Also note that the internal efficacy items were pre-faced with the following,

> "We are also interested in what people think of their own ability to understand and become involved in politics. I will read some statements. After each, please tell me whether you strongly agree with the statement, somewhat agree, neither agree nor disagree, somewhat disagree, or strongly disagree with the statement as it applies to you."

## 2025-11-20

I think it has become clear to me. 

There's a definite pattern that can be observed from the different attempts to conceptualize and distinguish the internal and external dimensions of political efficacy. In almost every case, the external dimension is simply the contrast established to better conceptualize and validate the internal dimension, whereas the internal dimension reflects the model of an ideal political character. The person with high internal political efficacy is confident in their own sense of political awareness and understanding and are generally more interested in public affairs; they deem themselves well qualified and generally well-read on politics and issues of the day; and they are not so intimidated by prospects of political engagement. This internal sense of political efficacy seems to stem from a more general sense of personal competence---a psychoanalytic atavism earlier scholars often referred to as "ego strength"---which is bolstered by higher education. So those high in internal efficacy are also better educated.

A set of characteristics consistently and strongly associated with internal efficacy excludes all else to the external dimension but leaves us wanting for a cohesive characterization of the "externally efficacious" citizen. In fact, the literature reveals that it is far easier to characterize those who lack external political efficacy than it is for us to imagine those who would be externally efficacious. 

Over the years scholars have searched in every undesirable place hoping to establish some link with external efficacy. Early on, a lack of external efficacy was, in theory, characteristic of apathy. The politically apathetic individuals who were described as withdrawn and disinterested in politics were expected to be those who also didn't participate. In other words, external efficacy explained a lack of engagement. This gets thrown on its head, however, as external efficacy in the aggregate continues to fall despite the tumultuous 60s and 70s.
